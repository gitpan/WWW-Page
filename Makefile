# This Makefile is for the WWW::Page extension to perl.
#
...
#     ABSTRACT_FROM => q[lib/WWW/Page.pm]
#     AUTHOR => q[Andrew Shitov <andy@shitov.ru>]
#     NAME => q[WWW::Page]
#     PREREQ_PM => { XSLT::Cache=>q[0], File::Cache::Persistent=>q[0], XML::LibXSLT=>q[0], XML::LibXML=>q[0] }
#     VERSION_FROM => q[lib/WWW/Page.pm]
...
TO_INST_PM = lib/WWW/Page.pm

PM_TO_BLIB = lib/WWW/Page.pm \
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) \
	  lib/WWW/Page.pm $(INST_MAN3DIR)/WWW::Page.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'name:                WWW-Page' >> META_new.yml
	$(NOECHO) $(ECHO) 'version:             1.99' >> META_new.yml
	$(NOECHO) $(ECHO) 'abstract:            XSLT-based and XML-configured website engine' >> META_new.yml
	$(NOECHO) $(ECHO) 'license:             ~' >> META_new.yml
	$(NOECHO) $(ECHO) 'author:              ' >> META_new.yml
	$(NOECHO) $(ECHO) '    - Andrew Shitov <andy@shitov.ru>' >> META_new.yml
	$(NOECHO) $(ECHO) 'generated_by:        ExtUtils::MakeMaker version 6.42' >> META_new.yml
	$(NOECHO) $(ECHO) 'distribution_type:   module' >> META_new.yml
	$(NOECHO) $(ECHO) 'requires:     ' >> META_new.yml
	$(NOECHO) $(ECHO) '    File::Cache::Persistent:       0' >> META_new.yml
	$(NOECHO) $(ECHO) '    XML::LibXML:                   0' >> META_new.yml
	$(NOECHO) $(ECHO) '    XML::LibXSLT:                  0' >> META_new.yml
	$(NOECHO) $(ECHO) '    XSLT::Cache:                   0' >> META_new.yml
...
ppd :
	...
	$(NOECHO) $(ECHO) '    <ABSTRACT>XSLT-based and XML-configured website engine</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Andrew Shitov &lt;andy@shitov.ru&gt;</AUTHOR>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="File-Cache-Persistent" VERSION="0,0,0,0" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="XML-LibXML" VERSION="0,0,0,0" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="XML-LibXSLT" VERSION="0,0,0,0" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="XSLT-Cache" VERSION="0,0,0,0" />' >> $(DISTNAME).ppd
...
pm_to_blib : $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', '\''$(PM_FILTER)'\'')' -- \
	  lib/WWW/Page.pm blib/lib/WWW/Page.pm 
